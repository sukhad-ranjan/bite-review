<?php

declare(strict_types=1);

require_once __DIR__ . '/../config.php';
require_once __DIR__ . '/../middleware/auth.php';
require_once __DIR__ . '/report_helpers.php';

if (($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST') {
    jsonResponse(['success' => false, 'message' => 'Method not allowed'], 405);
}

$token = getBearerToken();
$payload = validateJWT($token, JWT_SECRET);
if ($payload === false) {
    jsonResponse(['success' => false, 'message' => 'Unauthorized'], 401);
}

$input = readJsonInput();
$canteenId = sanitizeCanteenId((string) ($input['canteen_id'] ?? ''));
if ($canteenId === '') {
    jsonResponse(['success' => false, 'message' => 'Valid canteen_id is required'], 422);
}

if (($payload['canteen_id'] ?? '') !== $canteenId) {
    jsonResponse(['success' => false, 'message' => 'Forbidden'], 403);
}

$autoloadPath = dirname(__DIR__, 2) . '/vendor/autoload.php';
if (!file_exists($autoloadPath)) {
    jsonResponse(['success' => false, 'message' => 'Composer dependencies are not installed'], 500);
}

require_once $autoloadPath;

try {
    $pdo = getPDO();
    $week = (int) gmdate('W');
    $report = getWeeklyReport($pdo, $canteenId, $week);

    $adminStmt = $pdo->prepare('SELECT email FROM admins WHERE canteen_id = :canteen_id LIMIT 1');
    $adminStmt->execute([':canteen_id' => $canteenId]);
    $admin = $adminStmt->fetch();

    if (!$admin) {
        jsonResponse(['success' => false, 'message' => 'Admin email not found'], 404);
    }

    $mail = new \PHPMailer\PHPMailer\PHPMailer(true);
    $mail->isMail();
    $mail->setFrom('no-reply@bite-review.example.com', 'Bite Review');
    $mail->addAddress($admin['email']);
    $mail->isHTML(true);
    $mail->Subject = sprintf(
        'Bite Review - Weekly Report: %s - Week %d',
        $report['canteen_name'],
        $report['week']
    );

    $topItem = $report['top_items'][0]['item_name'] ?? 'No standout item yet';

    $mail->Body = sprintf(
        '<h2>%s Weekly Report</h2>
        <p>Week %d</p>
        <table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse;">
            <tr><th align="left">Total Reviews</th><td>%d</td></tr>
            <tr><th align="left">Average Rating</th><td>%.2f</td></tr>
            <tr><th align="left">Top Rated Item</th><td>%s</td></tr>
            <tr><th align="left">Complaint Count</th><td>%d</td></tr>
        </table>',
        htmlspecialchars($report['canteen_name'], ENT_QUOTES, 'UTF-8'),
        $report['week'],
        $report['total_reviews'],
        $report['avg_rating'],
        htmlspecialchars($topItem, ENT_QUOTES, 'UTF-8'),
        $report['complaint_count']
    );

    $mail->send();

    jsonResponse([
        'success' => true,
        'message' => 'Report sent to ' . $admin['email'],
    ]);
} catch (\PHPMailer\PHPMailer\Exception $exception) {
    jsonResponse(['success' => false, 'message' => 'Unable to send report email'], 500);
} catch (Throwable $exception) {
    jsonResponse(['success' => false, 'message' => 'Unable to send report email'], 500);
}
